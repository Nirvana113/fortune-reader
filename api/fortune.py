...
def handler(req):
    if req.method=='POST':
        try:
            d=req.get_json(); y=int(d.get('year',1990)); m=int(d.get('month',1)); d=int(d.get('day',1)); h=int(d.get('hour',12))
            img=gen(y,m,d,h); b=io.BytesIO(); img.save(b,'PNG'); b.seek(0); return send_file(b,mimetype='image/png')
        except Exception as e: return {'error':str(e)},500
    return {'status':'ok','message':'POST to /api/fortune with year,month,day,hour'}
