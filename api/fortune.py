...
def gen(y,m,d,h):
    ...
    dr.rectangle([30,fy+170,W-30,fy+220],fill=COLORS["bg_dark"]); dr.text((W//2,fy+185),f"Lucky Numbers: {' - '.join(map(str,ln))}",fill=COLORS["gold"])
    dr.text((W//2,H-40),"For Entertainment Only",fill=COLORS["text_dark"]); return img

def handler(req):
    if req.method=='POST':
        try:d=req.get_json(); y=int(d.get('year',1990)); m=int(d.get('month',1)); d=int(d.get('day',1)); h=int(d.get('hour',12))
            img=gen(y,m,d,h); b=io.BytesIO(); img.save(b,'PNG'); b.seek(0); return send_file(b,mimetype='image/png')
        except Exception as e: return jsonify({'error':str(e)}),500
    return jsonify({'status':'ok','message':'POST to /api/fortune with year,month,day,hour'})
